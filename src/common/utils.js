 ...
 * 微信授权
 **/
export const weChat = _shareObj => {
    let shareObj = _shareObj;
    // 测试环境不支持
    if (window.location.host.indexOf('www.moutechs.com') < 0) {
        return false
    }
    return axios({
        method: 'get',
        url: window.location.protocol + '//www.moutechs.com/wechat/outer/initJs?url=' + encodeURIComponent(shareObj.link),
    }).then(res => {
        if (res.data.code == 0) {
